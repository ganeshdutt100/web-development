...
<body style="font-family: Arial, sans-serif; background-color: #f8f9fa; text-align: center; padding: 50px;">
    ...
    <form action="data-submit" method="POST" style="max-width: 400px; margin: auto; background: white; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
    ...
    `);
    return res.end();
  } else if (req.url === "/data-submit" && req.method === "POST") {
    res.setHeader("Content-Type", "text/html");
    res.write(`
        <!DOCTYPE html>
<html lang="en">
...
<body style="font-family: Arial, sans-serif; background-color: #f8f9fa; text-align: center; padding: 50px;">
    ...
    try {
      fs.writeFileSync("data.text", "Ganesh Dutt");
